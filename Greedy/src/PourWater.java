...
//
//You can assume there's infinitely high terrain on the two sides out of bounds of the array.
    	...
    	while(V>0) {
    		...
    		for(int i=index; i>=0; i--) {
    			...
    			if(heights[i] > heights[index]) {
    				...
    				//当左边点比前面点低的时候 修改index到当前
    				index=i;
    			}
    		}
    		//如果在左边找到了合适位置 （不和初始值一样)
    		if(index != K) {
    			//水滴滴落到位置 所在位置的高度+1
    			heights[index]++;
    			//总共水滴量-1
    		...
    		//右边如果找到 或者左右两边都找到 在当前index滴落 高度+1
    		heights[index]++;
    		//总共滴水量-1
    		V--;
    	}
